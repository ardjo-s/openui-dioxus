...
if [ "$marker_observed" = true ] && kill -0 "$app_pid" 2>/dev/null && [ "$(uname -s)" = "Darwin" ]; then
  ...
  for _ in $(seq 1 20); do
    window_id=$(swift "$finder" "$app_pid" 2>/dev/null || true)
...
jq -n --argjson passed "$passed" --argjson evidence "$evidence_complete" --arg marker "$marker" --arg manifest "$manifest_hash" --arg binding "$binding_sha256" --argjson width "$width" --argjson height "$height" \
  '{platform:"desktop",routes:["openui","typed-json"],passed:$passed,evidence_complete:$evidence,marker:$marker,manifest_hash:$manifest,binding_sha256:$binding,screenshot:{file:"dioxus-desktop.png",width:$width,height:$height}}' > "$evidence/dioxus-desktop.json"
[ "$passed" = true ]
