...
const adapterSources = ["../../Cargo.toml", "../../Cargo.lock", "../../src/lib.rs", "../../src/catalog_evidence.rs", "../../src/rust_ui.rs", "../../src/rust_ui_upstream/mod.rs", ...["alert", "button", "card", "checkbox", "input", "label", "progress", "table"].map((name) => `../../src/rust_ui_upstream/${name}.rs`)];
const options = { manifestUrl, adapterSources, buildPrefix: "ope10" };
const sha = (value) => createHash("sha256").update(value).digest("hex");

function validateFixtureReferences(manifest) {
  ...
  const buttonActions = new Set(manifest.components.find((component) => component.name === "Button").props.action.enum);
  for (const fixture of Object.values(manifest.workflow_fixtures)) {
    ...
    for (const button of fixture.nodes.filter((node) => node.kind === "Button")) {
      assert.ok(actions.has(button.action), button.action);
      assert.ok(buttonActions.has(button.action), button.action);
    }
  }
}
...
test("Rust/UI source certifies eight components, six families, and two workflows", async () => {
  ...
  assert.ok(manifest.components.some((component) => component.name === "Table"));
  assert.ok(!manifest.components.some((component) => component.name === "Tabs"));
...
test("six frozen maintenance drills regenerate isolated contract artifacts reproducibly", async () => {
  const source = await loadManifest(manifestUrl);
  const sourceHash = sha(JSON.stringify(source));
  const drills = [
    ["component-prop", (next) => {
      const button = next.components.find((component) => component.name === "Button");
      button.prop_order.push("aria_description");
      button.props.aria_description = { type: "string", required: false };
      next.workflow_fixtures["01-profile-submit"].nodes.find((node) => node.kind === "Button").aria_description = "Submit the reviewed profile";
    }],
    ["component-schema", (next) => {
      next.components.push({ ...structuredClone(next.components.find((component) => component.name === "Alert")), name: "Callout", implementation: { module: "rust_ui::alert", component: "Alert", source_kind: "copied_upstream_component" } });
      const fixture = next.workflow_fixtures["01-profile-submit"];
      fixture.nodes.find((node) => node.kind === "Card").children.push("callout");
      fixture.nodes.push({ kind: "Callout", id: "callout", title: "Check", message: "Review complete.", tone: "info" });
    }],
    ["action-policy", (next) => {
      next.actions.push({ name: "archive_profile", input: { target_id: "string" } });
      next.components.find((component) => component.name === "Button").props.action.enum.push("archive_profile");
      next.workflow_fixtures["01-profile-submit"].nodes.find((node) => node.kind === "Button").action = "archive_profile";
    }],
    ["dtcg-token", (next) => { next.tokens.color.accent.$value = "#7c3aed"; }],
    ["catalog-release", (next) => { next.release_version = "0.0.2-ope10"; }],
    ["copy-on-write-migration", (next) => { next.catalog_contract_version = "0.1.1"; next.migrations = [{ from_manifest_hash: sourceHash, mode: "copy_on_write" }]; }],
  ];
  const run = async () => Promise.all(drills.map(async ([name, mutate]) => {
    ...
    validateFixtureReferences(next);
    ...
    return {
      ...
      fixture_references_checked: true,
  ...
  assert.ok(first.every((result) => !("behavior_checked" in result)));
  assert.ok(first.every((result) => result.contract_checked));
  assert.ok(first.every((result) => result.fixture_references_checked));
  ...
  assert.ok(first.every((result) => result.contract_checked && result.fixture_references_checked && result.registry_compiled));
